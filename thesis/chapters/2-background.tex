...
Neural networks can be adapted for many different tasks, and those tasks can be categorized by the type of input and the type of output. For example, spam detection is a “sequence classification” task in which the input is a sequence (in this case, of words) and the output is a boolean value (in this case, whether the input is a spam email). Translation is a sequence-to-sequence task, because the input sequence (e.g. a sentence in French) is mapped to an output sequence (e.g. an English translation of that sentence). The input and output sequences can differ from one another in length, in script, and in syntactic structure.
...
How are ideas, which the philosopher David Hume calls "perfect representations of impressions" (get actual quote) stored in our minds? There are no physical words, no objects within our crania, rather, billions of neurons which somehow come to work together to make representations of objects and concepts. Thus, to use language involves a symmetrical process. When we listen to speech or read words, our minds record the content and meaning in a completely different form than audio or text.\footnote{I'm no neuroscientist, but I take this to be true anyway - where else would ideas exist if not in our brains?}. To form verbal thoughts and to speak involves the reverse - making the abstract concrete, turning neural formations into letters, words, sounds. An artificial neural network can operate analogously; its input, e.g. text, can be encoded into an abstract representation (high dimensional vectors) and then decoded into whatever form its output should take, whether this be a boolean or string, a summary or a translation.  

The encoder-decoder architecture\cite{sutskever2014sequence} contains those two fundamental parts hinted at in the previous paragraph: an encoder and a decoder. The encoder operates on the input to produce an intermediate vector representation, and the decoder operates on the intermediate representation to produce output. This architecture could be adapted for a number of different tasks, but even in the paper which introduced encoder-decoders, a focus was placed on translation. The authors of that paper reported that, "While the decoded translations of the [encoder-decoder] ensemble do not outperform the best WMT’14 system, it is the first time that a pure neural translation system outperforms a phrase-based SMT [(Statistical Machine Translation)] baseline on a large scale MTtask by a sizeable margin, despite its inability to handle out-of-vocabulary words."

\section{The Transformer Model}

The Transformer model\cite{vaswani2017attention} innovated on the encoder-decoder structure, and has become the standard for NMT architecture. It uses an attention\cite{bahdanau2014neural} and positional encodings in place of recurrence. This new architecture trains much more quickly as it allows for more parallelizable training, and it is better at modeling long-range dependencies between sequence elements. Consider the following German sentence and its English translation:
...
The German version has the past participle "gesehen" (seen) at the very end of the sentence, separated from its corresponding modal verb "haben" (to have). This is a long-range dependency; accurate translation requires comprehension of the entire sentence, not just a word or two at a time. 

In the following section, I'll describe at a high level how a Transformer model translates a sentence.

\vskip.25in
\underline{Tokenization}
